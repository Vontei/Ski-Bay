...
module.exports = {
  ...
  /*
    find a product
      loop over array of category ids
      return product
      ---------------
        get the category name
          add new property to the product
            return product
  */

  categoriesForProduct: function (product) {
    return store.Categories.find({_id: {$in: product.category_id}})
  },
  ...
  getSellerAndProduct: function(reqUser, reqParamsId){
    return findProductById(reqParamsId).then(function(product){
      return Promise.all([
        this.getUserById(product.seller),
        this.categoriesForProduct(product)
      ]).then(function (data) {
        product.sellerObj = data[0]
        product.categories = data[1]
        return product
      })
    }.bind(this))
  },
